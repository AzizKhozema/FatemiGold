...
const TodoList = ({ transtasks, show }) => {
  const [tasks, setTasks] = useState(transtasks);
  ...
  const renderTask = tasks.map((task) => {
    return (
      ...
      >
        ...
        <div className="flex gap-4 items-center">
          <button className="iconbut editbut">
            <svg
          ...
          <button className="iconbut deletebut">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
    ...
    <>
      <div className="relative flex flex-col w-full h-full">
        <header className="flex justify-between content-center items-center">
          <h3 className="font-bold text-2xl ml-5 mt-2">Todo List</h3>
        ...
        <div className="taskList todocontainer overflow-y-scroll flex flex-col gap-2 mt-5 h-[100%] bg-gray-200 p-4 rounded-2xl shadow-md">
          {renderTask}
        </div>
      </div>
    </>
